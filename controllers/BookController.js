...
const destroyBook = (req, res) => {
    ...
    const selectQuery = `SELECT cover_image FROM books WHERE id = ?`;
    connection.query(selectQuery, [bookId], (err, selectResults) => {
        if (err) return res.status(500).json({ error: err });
        ...
        const coverImagePath = selectResults[0].cover_image
            ? path.join(__dirname, '../public', selectResults[0].cover_image)
            : null;

        const deleteQuery = `DELETE FROM books WHERE id = ?`;
        connection.query(deleteQuery, [bookId], (err, deleteResults) => {
            if (err) return res.status(500).json({ error: err });
            ...
            if (coverImagePath) {
                fs.unlink(coverImagePath, (err) => {
                    if (err) {
                        console.warn(
                            `⚠️ Could not delete image file: ${coverImagePath}`,
                            err.message
                        );
                    }
                });
